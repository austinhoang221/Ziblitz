...
export class AuthenticationService {
    public static logIn = async (payload: any) => {
        try {
        const response: IResponse<IAuthentication> = await axiosInstance.post(Endpoint.loginUrl, payload);
      ...
      public static signUp = async (payload: any) => {
        try {
          const response: IResponse<IAuthentication>  = await axiosInstance.post(Endpoint.signUpUrl, payload);
          console.log('POST response:', response.data);
          return response;
        } catch (error) {
          console.error('Error making POST request:', error);
